<template>
    <header class="admin-header">
        ...
        <div class="add-btn__wrapper">
            <el-button class="add-brand__btn" v-if="headers.hasSubMini" @click="(() => $router.push(`/admin/${headers.goSubMini}`))">
                {{ headers.addSubMini }}
            </el-button>
            ...
            <el-button class="add-product__btn" @click="(() => $router.push(`/admin/${headers.goRoute}`))">
                {{ headers.addMain }}
            </el-button>
        </div>
    </header>
...
.add-btn__wrapper .el-button:hover{
    background-color: #3D7BFE;
    color: #fff;
}
</style>